...
autoplot(db03, facets=TRUE) +
  xlab("") + ylab("") +
  ggtitle("Incoming bid and BI's intervention")
...
```{r}
# fit a Classical decomposition model for Bank Indonesia's intervention
db04_bi = db03[, "dv_bankindonesia"] %>% ts(., frequency = 12) # freq = 12 months
db04_bi_stl = stl(db04_dpk, s.window = "periodic")
autoplot(db04_dpk_stl)
```
...
incomingbidsun_arimax2 = auto.arima(
  ...
  # specify main trend
  xreg = db05_train[, "dv_bankindonesia"],
  # specify exogenous variables here
  trace = FALSE,
